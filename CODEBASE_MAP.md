...
| `investigate.ts` | The tool loop. Claude chases each finding and returns a verdict — confirmed / probably-fine / needs-human. Sonnet, thinking on. Verdicts arrive through a tool, not a final JSON blob, so a truncated run still yields what it settled. |
