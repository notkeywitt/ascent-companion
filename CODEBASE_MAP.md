...
| `budgetImport.ts` | "Import into JT": matches a tracking sheet's estimate to a job's live budget (name → cost → cost type → one-left, else the office chooses), prices from the page's markup (the API applies no margin), and writes the difference. Pure planner unit-tested in `budgetImport.test.ts`. |
