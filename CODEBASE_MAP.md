...
| **Coding / Client Invoicing workflow** | `src/app/recode/*` (Board, ClientInvoicing, DraftQueue, AllJobs, Roster) + `src/app/api/recode/*`, `src/app/api/code` |
...
- **Financials:** `recode` (Client Invoicing — the billing hub: Board,
  ClientInvoicing, DraftQueue, AllJobs, Roster), `bill/[docId]`, `add-bill`,
