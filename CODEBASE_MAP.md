...
| `taxableLines.ts` ⟂ | **The stray `isTaxable: false` worklist** behind `/taxable-lines` — bill lines whose flag takes their cost out of a client invoice's tax base, grouped per bill, biggest tax-at-stake first. READ-ONLY by design: a bulk update across live bills is the wrong shape for a tax decision, so it lists and links and a person clicks through. Two exclusions carry the whole risk — overhead jobs (never invoiced, and they were 58 of August's 75 lines) and sales-tax lines matched MORE strictly than `isSalesTaxLine` matches them, because that helper checks the code first and a line named "Sales Tax" coded `01 00 00` would otherwise read as ordinary and get taxed. Pure; the JobTread walk is `getUntaxedBillLines`. Unit-tested. |
...
- **Financials:** `trackingsheet` (Tracking Sheets — the billing hub, gated by
  ...
  `tracking-sheet`, `invoicing-summary` (Invoicing Package — the month's client
