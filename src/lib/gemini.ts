 ...
 *
 * SCOPE: the two paths that still run Gemini — bill extraction (`/api/add-bill`)
 * and tool-serial OCR (`/api/ocr-serial`). The digest's summary and email-signal
 * extraction used to live here too; they moved to Claude in `src/lib/digest/claude.ts`
 * and their Gemini twins were deleted 2026-09-05 rather than left as dead exports.
 */
