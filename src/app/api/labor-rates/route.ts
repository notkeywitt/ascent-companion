 ...
 * `name` is the JobTread pay-type name and the join key, so it's unique.
 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    const [rate] = await db
      ...
      .values({ name, hourlyRate, sortOrder: Number(body.sortOrder) || 0, createdAt: now, updatedAt: now })
      .returning();
    return NextResponse.json({ rate });
  } catch {
    return NextResponse.json({ error: `A rate named "${name}" already exists.` }, { status: 409 });
  }
}
